...
export default function Dashboard() {
  ...
  const [cashierStatus, setCashierStatus] =
  useState(null);
  ...
  const [todayPurchases, setTodayPurchases] =
  useState([]);
 ...
 useEffect(() => {
  ...
  const loadCashierStatus =
  async () => {
    try {
      const response =
        await axios.get(
          "http://localhost:5000/api/dashboard/cashier-status"
        );
  ...
  async () => {
    try {
      const response =
        await axios.get(
          "http://localhost:5000/api/dashboard/today-purchases"
        );
